...
const defaults: Record<string, (d?: { hasNext: boolean }) => States> = {
    ...
    CREATE_USER: (d?: { hasNext : boolean }) => ({ current: "CREATE_USER", hasNext: !!d?.hasNext, hasPrev: true }),
            ...
            return (authorized && !legacyToken 
                        ...
                        authenticated ? 
                            legacyToken ? 
                                defaults.CREATE_USER() : 
                                defaults.SUBSCRIBE() : 
                            defaults.AUTHENTICATE()) as States
        }
    },
    CREATE_USER: {
        NEXT: () => defaults.DONE(),
        ...
        UPDATE: ({ authorized, authenticated, user, formUsername }) => {
            return !authenticated ? defaults.SELECT_DOMAIN() : 
                (authorized && (["registered", "admitted"].includes(user?.status || ""))) 
                    ? defaults.DONE() : defaults.CREATE_USER({ hasNext: !!formUsername }) as States
        }
    },
    DONE: {
        UPDATE: ({ authorized }) => {
