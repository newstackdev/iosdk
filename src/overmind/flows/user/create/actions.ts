...
export const onInitializeOvermind: Action<any> = async ({ actions, effects, state, reaction }) => {
    const legacy: { form: UserCreateRequest, legacyToken: string, updated: number } =
    ...
    reaction(
        reduceState,
        actions.flows.user.create._wizardReact
    );
...
const usernameToAccount = (username: string) => { // adjust any string to newlife account name
...
export const _wizardReact: Action<WizardInput> = // ({ state, actions }, i: WizardInput) => 
    pipe(
        debounce(300),
        ({ state, actions }: Context, i: WizardInput) => {
            if (
...
export const preregisterCreate: Action<{ noRouting?: boolean, user?: UserCreateRequest }> =
...
export const create: Action<{ noRouting?: boolean, user: UserCreateRequest }> =
    async ({ actions }, params) => {
...
export const checkAvailability: Action<{ username: string }> =
    pipe(
