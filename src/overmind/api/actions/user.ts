import { ErrorResponse, MoodReadResponse, UserCreateRequest, UserInviteRequest, UserReadPublicResponse, UserUpdateRequest } from "@newlife/newlife-creator-client-api";
...
export const cache: Action<{ user: UserReadPublicResponse }> = async ({ state, actions, effects }, { user }) => {
...
export const read: Action<{ id?: string, username?: string }, UserReadPublicResponse> = async ({ state, actions, effects }, { id, username }) => {
    if (!(id || username))
        return;

    const known = (id && state.api.cache.users.byId[id]) || (username && state.api.cache.users.byUsername[username]);

    const ur: any = await (known ? Promise.resolve({ data: known }) : state.api.client.user.userList({ ...(id ? { id } : {}), username }));
...
export const create: Action<{ noRouting?: boolean, user: UserCreateRequest, preregisterCreate?: boolean }> =
    pipe(
        throttle(3000),
        async ({ state, effects, actions }: Context, { noRouting, user, preregisterCreate }) => {
            try {
    ...
    pipe(
        ...
        async ({ state, actions, effects }: Context, { user, amount }) => {
            try {
                ...
                const { error: { errorMessage } }: { error: { errorMessage: string, statusCode: number } } = ex as any;
