...
export const routeAfterAuth: Action<undefined> = async ({ state, actions }) => {
    ...
    const nextRoute =
        !state.api.auth.authorized ? "/user-create" :
...
export const onRouteChange: Action<{ location: { pathname: string, search: string } }> = async ({ state, actions }, { location: { pathname, search } }) => {
...
export const historyPush: Action<{ location: string, force?: boolean }> = async ({ state, effects }, { location, force }) => {
