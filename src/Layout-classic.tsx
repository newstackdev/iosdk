import { Layout as AntdLayout, Breadcrumb, Col, Menu, Row, notification } from "antd";
    ...
    <AntdLayout>
      <Header key="h" className="header logo">
        <Menu theme="dark" mode="horizontal" defaultSelectedKeys={["2"]}>
          ...
          <Menu.Item key="1">
            <Link to="/explore" /*underlayColor="#f0f4f7"*/ className="nav-item">
              {/* ${state.indicators.inProgress ? "rotating" : ""} */}
              <div className={`logo-image`}></div>
            </Link>
          </Menu.Item>
          <Menu.Item key="0">
            <Link to="/payment/subscription" /*underlayColor="#f0f4f7"*/ className="nav-item">
              <div>Subscribe</div>
            </Link>
          </Menu.Item>
          <Menu.Item key="2">
            <Link to="/about" /*underlayColor="#f0f4f7"*/ className="nav-item">
              About
            </Link>
          </Menu.Item>
          <Menu.Item key="3">
              ...
              state.auth.status < AUTH_FLOW_STATUS.AUTHENTICATED ? (
                ...
                <>
                  <Breadcrumb style={{ margin: "16px 0" }}>
                    {
                      //@ts-ignore
                      state.routing.breadcrumbs.map((b, i, a) => (
                        <>
                          {b.url ? (
                            <Breadcrumb.Item>{b.url ? <Link to={b.url}>{b.text}</Link> : b.text}</Breadcrumb.Item>
                          ) : (
                            <Breadcrumb.Item>{b.text}</Breadcrumb.Item>
                          )}
                        </>
                      ))
                    }
                    {state.routing.breadcrumbs.length ? <Breadcrumb.Item>&gt;</Breadcrumb.Item> : ""}
                  </Breadcrumb>
