...
export const UserSearchResultsWidget: NLView<{ query: string }> = ({ query }) => {
  ...
  return (
    <ul className="nl-white-box app-box-shadow paragraph-1r user-search-results-widget">
...
export const SearchWidget: NLView<{
  ...
  const [SearchResultsWidget, setSearchResultsWidget] = useState<{
    component: SearchResultsWidget;
  }>({ component: TagsAutosuggestWidget });
  const actions = useActions();
          ...
          <Col span={20} className="app-main-full-width-only">
            {search && (
              <Dropdown
                ...
                overlay={<SearchResultsWidget.component query={query || ""} />}
              >
