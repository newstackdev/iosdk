...
export const MoodsGridRow: NLView<{
  ...
  useEffect(() => {
    ...
    isVisible && mood && (pc === 0 || (pc < 4 && !maxItems)) && actions.api.mood.getPosts(mood);
  }, [isVisible]);
  ...
  return (
    <Row
      ref={currentElement}
      wrap={wrap}
      className={`nl-mood-grid-row  ${noFullWidth ? "nl-mood-grid-row-height" : "app-main-full-width"} ${wrap ? "wrap" : ""}`}
        ...
        <Col xs={8}>
          <div style={{ width: "100%" }}>
            <Link to={`/folder/${mood.id}`}>
              <FolderClosed width="min(66px,100%)" height="auto" className="text-center" />
              <br />
...
export const MoodsGrid: NLView<MoodsGridParams> = ({ moods, title, loadMore }) => {
