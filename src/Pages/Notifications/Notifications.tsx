  ...
  return (
    <Row className="notification-box" justify="space-between">
      <Row className="notification-box__inside-left">
        ...
        <Col sm={13} md={17} lg={17} xl={18} xxl={18} className="notification-box__inside-left-col-2">
          <Col style={{ height: "auto" }}>
            <span className="paragraph-2b">newdomain.io </span>
            <span className="paragraph-2r notification-box__inside--action-info">added your...</span>
          </Col>
          <Col style={{ height: "auto" }} className="notification-box__inside-left--time">
            <p className="paragraph-2r">Now</p>
          </Col>
        </Col>
      </Row>
      <Row className="notification-box__inside-right text-right" justify="end">
        <Col sm={6} className="notification-box__inside-right--grey-box">
          <PostWidget mood={mood} post={post} username={username} aspectRatio={aspectRatio} />
        </Col>
...
export const Notifications: NLView = () => {
  ...
  return (
    <ContentLayout>
      <Row style={{ width: "100%", margin: "20px 0" }}>{/* <LargeArrowBack /> */}</Row>
      <div className="notifications-wrapper ">
        {postsList?.slice(0, 20).map((p: any, i: number) => (
          <NotificationBox user={user} mood={postsList[i]} post={p} username={p.author?.username} aspectRatio={p.aspectRatio} />
        ))}
      </div>
    </ContentLayout>
  );
