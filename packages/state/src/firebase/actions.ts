...
export const onInitializeOvermind: Action<undefined> = async ({
  ...
  const auth = effects.firebase.initialize(
    state.config.settings.firebaseConfig
  );
...
export const refreshApiToken: Action = async ({ state, actions, effects }) => {
  const token = state.firebase.user
    ? await state.firebase.user.getIdToken(true)
    : "";

  if (!token) return actions.auth.logout();

  state.auth.status = Math.max(
...
export const handleAuthChange: Action<User | null> = async (
...
export const requestEmailLink: Action<{ email: string }> = async (
...
export const signInWithEmailLink: Action<{ email: string }, boolean> = async ({
  state,
  actions,
  effects,
}) => {
...
export const requestToken: Action<{ phone: string }> = async (
  { state, actions, effects },
  { phone }
) => {
  ...
  const timeout = setTimeout(() => {
    ...
    if (state.auth.status != AUTH_FLOW_STATUS.AUTHENTICATED) {
      effects.ux.message.warning(
        "Authentication was taking too long. Please try again."
      );
      actions.auth.logout();
    }
  }, 120 * 1000);
...
export const setFbUser: Action<{ user: User }> = async (
  { state, actions, effects },
  { user }
) => {
