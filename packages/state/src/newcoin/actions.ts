...
export const getPoolInfo: Action<{ pool: { owner?: string; code?: string } }> =
  pipe(debounce(200), async ({ effects, state }: Context, { pool }) => {
...
export const getAccountHitory: Action<
  { user?: { username?: string }; force?: boolean },
  any
> =
  // pipe(
  // debounce(1000),
  async ({ effects, state }: Context, { user, force }) => {
    ...
    const r = await effects.newcoin.hyperion(
      `/v2/state/get_account?account=${user?.username}`
    );
    return (state.newcoin.cache.accountHistory[user?.username || ""] =
      (await r.json()) as any);
  };
// )
