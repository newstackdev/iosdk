import { EmbeddableControl, APP_DOMAIN } from "@newcoin-foundation/core";
...
export const JoinDaoWidget = ({
  ...
  const justStaked = [...state.api.cache.stakeHistory]
    .reverse()
    .find((h) => h.user.username === APP_DOMAIN);
  ...
  return (
    ...
    >
      ...
      {isMember ? (
        <>
          <h2 className="text-left text-bold">
            You are a member of the Newlife DAO
          </h2>
          <div style={{ textAlign: "left" }}>
            You may increase your stake or visit the{" "}
            <Link to={`/user/${APP_DOMAIN}`}>${APP_DOMAIN}</Link> home page.
          </div>
        </>
      ) : (
        <>
          ...
          >
            Newlife is governed by the community. To join the app, you need to
            own at least 1000 $LIFE on your account.
          </p>
          ...
          <p
            ...
            className="paragraph-1r"
          >
            By clicking "Stake" you will deposit 1087 $NCO into the DAO to
            receive 1000 $LIFE tokens. You can recover them anytime minus the
            stake 10% fee.
          </p>
