...
const PostWidget = ({ post, mood, username, aspectRatio, isSpotlight }) => {
    const p = post;
    return ((0, jsx_runtime_1.jsx)(jsx_runtime_1.Fragment, { children: (0, jsx_runtime_1.jsx)(MaybeLink, { to: !p.id
                ? ""
                : !mood
                    ? `/post/${p.id}`
                    : `/folder/${mood.id}/${p.id}`, className: p.contentType === "text/plain" ? "maybelink" : "", children: p.contentType === "text/plain" ? ((0, jsx_runtime_1.jsx)("div", { className: "text-container", children: (0, jsx_runtime_1.jsxs)("p", { className: "text-mood", children: ["\"", p.content?.length > 17
                            ? p?.content?.substring(0, 12) + "..."
                            : p.content || "", "\""] }) })) : ((0, jsx_runtime_1.jsx)(Image_1.ContentImage, { size: "small", ...p, width: "100%", style: { aspectRatio: `${aspectRatio}` }, className: isSpotlight || aspectRatio >= 1
                    ? "post-rounded"
                    : "" })) }) }));
    // return <Card
    //     style={{ width: "100%" }}
