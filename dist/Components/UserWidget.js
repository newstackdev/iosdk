    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Modal, { closeIcon: _jsx(CrossCircle, {}), 
                ...
                onCancel: () => {
                    ...
                    setMode(STAKE_STEPS.DISABLED);
                                        ...
                                        [ncoBalance]: "100%",
                                    }, min: minValue / minValue, max: ncoBalance }) }) }), _jsx(Col, { span: 24, className: "u-margin-top-large", children: _jsx(Button, { className: "nl-button-primary stake-button-modal", onClick: () => {
...
export const UserWidgetHeading = ({ user, setActiveKey, activeKey }) => {
    ...
    let poweredNumber = u.powered || 0;
    let poweringNumber = u.powering || 0;
...
export const UserNewcoinInfo = ({ user = {} }) => {
    ...
    const { verifiedUsers } = useVerified([user.username || ""]);
    const isUserVerified = verifiedUsers && user.username && verifiedUsers.includes(user.username);
    return (_jsxs(_Fragment, { children: [_jsx(DataRow, { title: "newcoin domain name", value: isUserVerified ? (_jsxs("div", { style: { display: "inline-flex", alignItems: "center" }, children: [_jsx("span", { className: "u-margin-right-medium", children: user.username }), _jsx(VerifiedIcon, {})] })) : (user.username), link: `https://explorer-dev.newcoin.org/account/${user.username}`, collapse: true }), _jsx(DataRow, { title: "newcoin ticker", value: user.newcoinTicker.toUpperCase(), collapse: true }), _jsx(DataRow, { title: "account balance", value: state.newcoin.account.acc_balances, collapse: true }), _jsx(DataRow, { title: "newcoin pool", value: _jsx(BlockExplorerLink, { id: user.newcoinPoolId }), collapse: true }), _jsx(DataRow, { title: "newcoin account", value: _jsx(BlockExplorerLink, { id: user.newcoinAccTx }), collapse: true }), _jsx(DataRow, { title: "newcoin publisher public key", value: _jsx(HashDisplay, { hash: user.newcoinPublisherPublicKey }), collapse: true }), _jsx(DataRow, { title: "newcoin publisher private key", value: _jsx(RevealInfo, { children: _jsx(HashDisplay, { hash: user.newcoinPublisherPrivateKey }) }), collapse: true })] }));
};
