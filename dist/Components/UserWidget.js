...
export const UserStake = ({ user, mode, value, minValue, hideButton, buttonText, hideSelect, closeOnDone, onDone, onCancel, usePowerupTerminology = false, stakeInNewsafe, }) => {
    ...
    return (_jsxs(_Fragment, { children: [_jsxs(Modal, { closeIcon: _jsx(CrossCircle, {}), 
                ...
                onCancel: () => {
                    ...
                    e.preventDefault();
                    e.stopPropagation();
                    showPopUp(`https://auth${state.config.env.env == "dev" ? "-dev" : ""}.newsafe.org/swap/GNCO/${user?.newcoinTicker?.toUpperCase()}`, "__NEWSAFE__");
                }, children: buttonText || "Stake" })) : (_jsx(ProgressButton, { actionName: "api.user.stake", onClick: () => {
                    closeWallet();
                    startStaking();
