...
export const Wizard = statemachine({
    ...
    SELECT_DOMAIN: {
        ...
        UPDATE: ({ authorized, formUsername, formUsernameIsAvailable, user, featureFlags, subscribed }) => {
    ...
    CREATE_USER: {
        ...
        PREV: ({ authenticated, formUsername }) => {
            return (authenticated ? { ...defaults.SELECT_DOMAIN(), hasNext: !!formUsername } : { ...defaults.AUTHENTICATE(), hasNext: false });
        },
        UPDATE: ({ authorized, authenticated, user, formUsername, isLegacyUpdateOngoing }) => {
