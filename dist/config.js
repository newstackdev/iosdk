...
const canonicalHosts = {
    ...
    "eu-prod": process.env.REACT_APP_IOSDK_APP_PROD_CANONICAL_HOSTS ||
...
const daoDomain = process.env[`REACT_APP_IOSDK_APP_DAO_DOMAIN_${env.toUpperCase()}`] ||
...
export const canonicalHostsForStage = canonicalHosts[stage] || [];
console.log("Stage", stage);
console.log("Firebase config", firebaseConfig);
// This should not be happening in mainnet
const displayDaoDomain = process.env[`REACT_APP_IOSDK_DISPLAY_DAO_DOMAIN_${env.toUpperCase()}`] ||
    process.env["REACT_APP_IOSDK_DISPLAY_DAO_DOMAIN"] ||
    daoDomain;
if (!daoDomain)
    console.warn("IOSDK: settings.app.daoDomain not configured, use REACT_APP_IOSDK_APP_DAO_NAME in .env to set to app owner's username.");
